...
local BossEnemy = Entity:extend()

function BossEnemy:new()
    self.super.new(self)
    self.x = 0
    self.y = 0
    self.width = 50
    ...
    self.MAX_HP = 50
    self.HP = self.MAX_HP
    self.Attack = math.random(10, 30)
    self.Block = math.random(5, 15)
    self.intent = "Attacking for " .. self.Attack
end

function BossEnemy:action()
...
function BossEnemy:draw(x, y)
    if self.alive then
        love.graphics.setColor(1, 0, 1)
        love.graphics.rectangle("fill", x, y, 50, 50)
        love.graphics.setColor(1, 1, 1)
        BossEnemy:drawHealthBar(x - 25, y + 60)
        love.graphics.print(
            "Block: " .. self.Armor,
            x - (self.width / 5),
            y + self.height + 20
